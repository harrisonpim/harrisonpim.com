...
As any scientist knows, producing the same results by doing the same thing again and again is actually surprisingly hard! In practice, I think it's more true to say:
...
As machine learning practitioners, we build a lot of powerful decision-making systems. However, I rarely see people calculating or communicating uncertainties in this field!
...
While it might be true that some values fall at the classification boundary between 0 and 1 because the model is uncertain about which class the  However, there will invariably be cases where .

Some of the cases close to the boundary are uncertain, reflecting a poor understanding of the feature space. With each forward pass, it could classify the observation in either bucket

However, some predictions sit *confidently* at the boundary between the two classes. The model knows that the data it's seeing falls somewhere between the two classes and will always put it in that spot.. 
...
In my experience, this illustration is much closer to the reality than the previous one.
...
Luckily for us, the [laplace package](https://github.com/aleximmer/Laplace) provides a really easy way to get started with 
...
- **[Deep ensembles](https://arxiv.org/abs/1612.01474):** A higher level version of Monte Carlo dropout - Deep ensembles involve training multiple neural network models independently on the same data and then aggregating their predictions. This approach provides a measure of uncertainty by examining the variability in predictions across the ensemble members. It has been shown to be effective in capturing epistemic uncertainty, particularly when combined with techniques like temperature scaling and Monte Carlo dropout.
